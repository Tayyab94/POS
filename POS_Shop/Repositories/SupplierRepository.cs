using POS_Shop.DTOs.Supplier;
using POS_Shop.Interfaces;
using POS_Shop.Models;
using System;
using System.Collections.Generic;
using System.Data.Entity;
using System.Linq;
using System.Threading.Tasks;

namespace POS_Shop.Repositories
{
    public class SupplierRepository : Repository<Supplier>, ISupplierRepository
    {
        public SupplierRepository(POSDbContext context) : base(context)
        {
        }

        public async Task<bool> CheckRecoradAlreadyExistByName(string name, string address) => await _context.Customers.AnyAsync(x => x.CustomerName.Equals(name, StringComparison.OrdinalIgnoreCase) && x.CustomerAddress.Equals(address, StringComparison.OrdinalIgnoreCase));

        public async Task<IEnumerable<Supplier>> GetAll(List<int> ids)
        {
            var data = await _context.Suppliers.Where(e => ids.Contains(e.Id)).ToListAsync();
            return data;
        }

        public async Task<(int totalCount, IEnumerable<SupplierListForDataGridDto> data)> GetSupplierPagingListAsync(int pageIndex, int pageSize, string search)
        {
            var data = _context.Suppliers
                .Include(s => s.City)
                .AsNoTracking()
                .AsQueryable();

            // Apply search filter if provided
            if (!string.IsNullOrWhiteSpace(search))
            {
                // Split once and filter out empty entries
                var searchWords = search.Trim()
                    .ToLower()
                    .Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);

                // Build predicate efficiently
                foreach (var word in searchWords)
                {
                    string currentWord = word; // Capture variable for closure
                    data = data.Where(s =>
                        s.SupplierName.ToLower().Contains(currentWord) ||
                        s.Address.ToLower().Contains(currentWord) ||
                        s.ShopName.ToLower().Contains(currentWord) ||
                        s.City.Name.ToLower().Contains(currentWord)
                    );
                }
            }

            var totalCount = await data.CountAsync();
            var result = await data.OrderByDescending(s => s.Id)
                .Skip((pageIndex - 1) * pageSize)
                .Take(pageSize)
                .Select(s => new SupplierListForDataGridDto()
                {
                    Id = s.Id,
                    SupplierName = s.SupplierName,
                    ContactNo = s.ContactNo,
                    ShopName = s.ShopName,
                    Address = s.Address,
                    CityName = s.City.Name,
                    IsDeleted = s.IsDeleted,
                    CityId = s.CityId
                }).ToListAsync();

            return (totalCount, result);
        }
    }
}
