...
//                    .OrderByDescending(p => p.CreatedAt)   // oldest first
//                    .ToList();
...
//            // ── AUTO SYNC CHECKBOX ─────────────────────────────────────────────
//            // Amount > 0 → check the row.  Amount = 0 → uncheck the row.
//            // _suppressGridEvent prevents CellValueChanged from overwriting the
//            // amount we just set.
//            _suppressGridEvent = true;
//            row.Cells["colSelect"].Value = (amt > 0);
//            _suppressGridEvent = false;
...
//            txtTotalAmt.Text = totalAlloc.ToString("N2");
...
//        // ══════════════════════════════════════════════════════════════════════
//        //  CHECKBOX — commit instantly when user clicks (no need to leave cell)
//        // ══════════════════════════════════════════════════════════════════════

//        /// <summary>
//        /// Forces the checkbox to commit its new value as soon as it is clicked,
//        /// so CellValueChanged fires immediately rather than on focus loss.
//        /// </summary>
...
//        /// <summary>
//        /// When colSelect checkbox changes:
//        ///   Checked   → copy Balance value into Allocate column
//        ///   Unchecked → set Allocate to 0.00
//        /// Then refresh the summary totals.
//        /// </summary>
//        private void DgvInvoices_CellValueChanged(object sender, DataGridViewCellEventArgs e)
//        {
//            if (e.RowIndex < 0) return;
//            if (dgvInvoices.Columns[e.ColumnIndex].Name != "colSelect") return;
...
//            {
//                decimal balance = D(row.Cells["colBalance"].Value?.ToString());
//                row.Cells["colAllocate"].Value = balance.ToString("N2");
//            }
//            else
//            {
//                row.Cells["colAllocate"].Value = "0.00";
...
//        /// Select All  → checks every row (fills Balance → Allocate)
//        /// Deselect All → unchecks every row (sets Allocate to 0.00)
//        /// </summary>
//        private void BtnSelectAll_Click(object sender, EventArgs e)
//        {
//            if (dgvInvoices.Rows.Count == 0) return;

//            // Determine current state: if any row is unchecked, select all; else deselect all
//            bool anyUnchecked = false;
//            foreach (DataGridViewRow row in dgvInvoices.Rows)
//            {
//                bool chk = row.Cells["colSelect"].Value is bool b && b;
//                if (!chk) { anyUnchecked = true; break; }
//            }

//            bool newState = anyUnchecked;   // true = check all,  false = uncheck all

//            _suppressGridEvent = true;
//            foreach (DataGridViewRow row in dgvInvoices.Rows)
//            {
//                row.Cells["colSelect"].Value = newState;
//                if (newState)
//                {
//                    decimal balance = D(row.Cells["colBalance"].Value?.ToString());
//                    row.Cells["colAllocate"].Value = balance.ToString("N2");
...
//            btnSelectAll.Text = newState ? "☐  Deselect All" : "☑  Select All";
...
//                case Keys.Control | Keys.N: btnCancel.PerformClick(); return true;
//                case Keys.Escape:
//                    txtSupSearch.Focus(); txtSupSearch.SelectAll(); return true;
...
namespace POS_Shop.Views.Controllers.Supplier
{
    ...
    public partial class SupplierPaymentForm : Form
    {
        ...
        //    decimal bal = D(row.Cells["colBalance"].Value?.ToString());
        //    //colNetAmt
        //    decimal netAmt = D(row.Cells["colNetAmt"].Value?.ToString());
        ...
        private void DgvInvoices_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            // Auto-sync checkbox based on allocation amount
            _suppressGridEvent = true;
            row.Cells["colSelect"].Value = (amt > 0);
            _suppressGridEvent = false;
        ...
        //        row.Cells["colBalance"].Value = netAmt;
        //        row.Cells["colAllocate"].Value = "0.00";
        //    }
        //    _suppressGridEvent = false;
        ...
        private void DgvInvoices_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (isChecked)
            {
                // Only auto-fill if allocate is 0
                decimal currentAlloc = D(row.Cells["colAllocate"].Value?.ToString());
                if (currentAlloc == 0)
                {
                    decimal balance = D(row.Cells["colBalance"].Value?.ToString());
                    row.Cells["colAllocate"].Value = balance.ToString("N2");
                }
                // If allocate already has a value, keep it
            }
            else
            {
                // Unchecked: set allocate to 0
                row.Cells["colAllocate"].Value = "0.00";
            }
            _suppressGridEvent = false;
